...
def on_prompt(json_data):
    ...
    for k, v in prompt.items():
        if 'class_type' in v and (v['class_type'] in ['PromptUtilitiesRandomPreset','PromptUtilitiesRandomPresetAdvanced']):
            inputs = v['inputs']
            print('before:',inputs)
            input_seed = int(inputs['seed'])
            input_filename = inputs['filename']
            if v['class_type'] == 'PromptUtilitiesRandomPreset':
                inputs['choice_preset'] = PresetManager.random_preset(input_filename, input_seed)
            ...
            server.PromptServer.instance.send_sync("prompt-utilities-feedback", {"node_id": k, "widget_name": "choice_preset", "type": "STRING", "value": inputs['choice_preset']})
